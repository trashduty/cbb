# CBB Model Record

**Last Updated:** January 17, 2026 at 04:49 PM ET

**Season Record Period:** 2025-11-04 to 2026-01-17

**Total Games Graded:** 1113 games (2226 team-rows)

---

## Record by Opening Edge Tier

The opening edge is calculated when a game first appears in our system, comparing our model's predicted probability against the market's implied probability at that moment.

### Spread Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 131-120 | 52.2% | -0.92 |
| 2-3.9% | 41-50 | 45.1% | -12.73 |
| 4-5.9% | 18-17 | 51.4% | -0.64 |
| 6%+ | 5-9 | 35.7% | -4.46 |
| **Total** | **195-196** | **49.9%** | **-18.75** |

### Spread Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 31-44 | 41.3% | -15.82 |
| 2-3.9% | 21-20 | 51.2% | -0.91 |
| 4-5.9% | 7-8 | 46.7% | -1.64 |
| 6%+ | 2-7 | 22.2% | -5.18 |
| **Total** | **61-79** | **43.6%** | **-23.55** |

### Moneyline Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 81-111 | 42.2% | -37.37 |
| 2-3.9% | 13-17 | 43.3% | -5.18 |
| 4-5.9% | 0-6 | 0.0% | -6.00 |
| 6%+ | 1-4 | 20.0% | -3.09 |
| **Total** | **95-138** | **40.8%** | **-51.64** |

### Moneyline Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 46-57 | 44.7% | -15.19 |
| 2-3.9% | 5-8 | 38.5% | -3.46 |
| 4-5.9% | 0-4 | 0.0% | -4.00 |
| 6%+ | 0-1 | 0.0% | -1.00 |
| **Total** | **51-70** | **42.1%** | **-23.65** |

### Over Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 42-40 | 51.2% | -1.82 |
| 2-3.9% | 2-4 | 33.3% | -2.18 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 2-0 | 100.0% | +1.82 |
| **Total** | **46-44** | **51.1%** | **-2.18** |

### Over Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 6-6 | 50.0% | -0.55 |
| 2-3.9% | 0-2 | 0.0% | -2.00 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **6-8** | **42.9%** | **-2.55** |

### Under Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 70-70 | 50.0% | -6.37 |
| 2-3.9% | 10-8 | 55.6% | +1.09 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 2-0 | 100.0% | +1.82 |
| **Total** | **82-78** | **51.2%** | **-3.46** |

### Under Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 18-16 | 52.9% | +0.36 |
| 2-3.9% | 6-2 | 75.0% | +3.45 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **24-18** | **57.1%** | **+3.81** |

---

## Individual Model Performance

Performance of each individual predictive model, calculated across all games regardless of edge.

### Individual Model Performance - Spread Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 1025-1009 | 50.4% | -77.27 |
| Bart Torvik | 684-680 | 50.1% | -58.24 |
| Evan Miya | 1085-1123 | 49.1% | -136.74 |
| Haslametrics | 1048-1092 | 49.0% | -139.37 |

### Individual Model Performance - Moneyline Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 1488-564 | 72.5% | +788.59 |
| Bart Torvik | 1020-344 | 74.8% | +583.18 |
| Evan Miya | 1616-610 | 72.6% | +858.94 |

### Individual Model Performance - Over Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 1024-962 | 51.6% | -31.18 |
| Bart Torvik | 686-630 | 52.1% | -6.43 |
| Evan Miya | 1116-1040 | 51.8% | -25.56 |
| Haslametrics | 1086-1008 | 51.9% | -20.83 |

### Individual Model Performance - Under Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 962-1024 | 48.4% | -149.54 |
| Bart Torvik | 630-686 | 47.9% | -113.33 |
| Evan Miya | 1040-1116 | 48.2% | -170.64 |
| Haslametrics | 1008-1086 | 48.1% | -169.73 |

---

## Methodology

- **Opening Edge**: The difference between our model's predicted probability and the market's implied probability when the game first appeared in our system
- **Consensus**: All individual models agree on the direction of the bet
- **Profit Calculation**: Assumes flat betting at -110 standard juice (+0.909 units per win, -1 unit per loss)
- **Edge Tiers**: Games are bucketed by their opening edge percentage
- **Individual Model Performance**: Each model's performance across all games, regardless of edge

## Notes

- This report updates automatically every 15 minutes
- Only completed games with valid opening edges are included
- Games without opening edge data (e.g., missing from initial scrape) are excluded from this report

