# CBB Model Record

**Last Updated:** January 02, 2026 at 01:05 AM ET

**Season Record Period:** 2025-11-03 to 2026-01-02

**Total Games Graded:** 399 games (798 team-rows)

---

## Record by Opening Edge Tier

The opening edge is calculated when a game first appears in our system, comparing our model's predicted probability against the market's implied probability at that moment.

### Spread Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 60-41 | 59.4% | +13.54 |
| 2-3.9% | 19-26 | 42.2% | -8.73 |
| 4-5.9% | 16-11 | 59.3% | +3.54 |
| 6%+ | 8-11 | 42.1% | -3.73 |
| **Total** | **103-89** | **53.6%** | **+4.62** |

### Spread Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 19-16 | 54.3% | +1.27 |
| 2-3.9% | 13-13 | 50.0% | -1.18 |
| 4-5.9% | 7-7 | 50.0% | -0.64 |
| 6%+ | 2-7 | 22.2% | -5.18 |
| **Total** | **41-43** | **48.8%** | **-5.73** |

### Moneyline Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 65-93 | 41.1% | -33.92 |
| 2-3.9% | 6-9 | 40.0% | -3.55 |
| 4-5.9% | 0-5 | 0.0% | -5.00 |
| 6%+ | 1-1 | 50.0% | -0.09 |
| **Total** | **72-108** | **40.0%** | **-42.56** |

### Moneyline Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 46-52 | 46.9% | -10.19 |
| 2-3.9% | 4-7 | 36.4% | -3.36 |
| 4-5.9% | 0-4 | 0.0% | -4.00 |
| 6%+ | 0-1 | 0.0% | -1.00 |
| **Total** | **50-64** | **43.9%** | **-18.55** |

### Over Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 6-6 | 50.0% | -0.55 |
| 2-3.9% | 0-0 | N/A | +0.00 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **6-6** | **50.0%** | **-0.55** |

### Over Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 2-2 | 50.0% | -0.18 |
| 2-3.9% | 0-0 | N/A | +0.00 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **2-2** | **50.0%** | **-0.18** |

### Under Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 32-20 | 61.5% | +9.09 |
| 2-3.9% | 6-4 | 60.0% | +1.45 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 2-0 | 100.0% | +1.82 |
| **Total** | **40-24** | **62.5%** | **+12.36** |

### Under Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 8-12 | 40.0% | -4.73 |
| 2-3.9% | 6-0 | 100.0% | +5.45 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **14-12** | **53.8%** | **+0.72** |

---

## Individual Model Performance

Performance of each individual predictive model, calculated across all games regardless of edge.

### Individual Model Performance - Spread Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 347-363 | 48.9% | -47.58 |
| Bart Torvik | 368-346 | 51.5% | -11.49 |
| Evan Miya | 378-418 | 47.5% | -74.40 |
| Haslametrics | 369-379 | 49.3% | -43.58 |

### Individual Model Performance - Moneyline Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 563-149 | 79.1% | +362.77 |
| Bart Torvik | 584-130 | 81.8% | +400.86 |
| Evan Miya | 642-156 | 80.5% | +427.58 |

### Individual Model Performance - Over Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 328-338 | 49.2% | -39.85 |
| Bart Torvik | 306-356 | 46.2% | -77.85 |
| Evan Miya | 356-390 | 47.7% | -66.40 |
| Haslametrics | 334-370 | 47.4% | -66.39 |

### Individual Model Performance - Under Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 338-328 | 50.8% | -20.76 |
| Bart Torvik | 356-306 | 53.8% | +17.60 |
| Evan Miya | 390-356 | 52.3% | -1.49 |
| Haslametrics | 370-334 | 52.6% | +2.33 |

---

## Methodology

- **Opening Edge**: The difference between our model's predicted probability and the market's implied probability when the game first appeared in our system
- **Consensus**: All individual models agree on the direction of the bet
- **Profit Calculation**: Assumes flat betting at -110 standard juice (+0.909 units per win, -1 unit per loss)
- **Edge Tiers**: Games are bucketed by their opening edge percentage
- **Individual Model Performance**: Each model's performance across all games, regardless of edge

## Notes

- This report updates automatically every 15 minutes
- Only completed games with valid opening edges are included
- Games without opening edge data (e.g., missing from initial scrape) are excluded from this report

