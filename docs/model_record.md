# CBB Model Record

**Last Updated:** January 11, 2026 at 03:05 PM ET

**Season Record Period:** 2025-11-03 to 2026-01-11

**Total Games Graded:** 898 games (1796 team-rows)

---

## Record by Opening Edge Tier

The opening edge is calculated when a game first appears in our system, comparing our model's predicted probability against the market's implied probability at that moment.

### Spread Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 107-102 | 51.2% | -4.74 |
| 2-3.9% | 38-43 | 46.9% | -8.46 |
| 4-5.9% | 20-15 | 57.1% | +3.18 |
| 6%+ | 10-12 | 45.5% | -2.91 |
| **Total** | **175-172** | **50.4%** | **-12.93** |

### Spread Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 28-41 | 40.6% | -15.55 |
| 2-3.9% | 21-20 | 51.2% | -0.91 |
| 4-5.9% | 7-8 | 46.7% | -1.64 |
| 6%+ | 2-7 | 22.2% | -5.18 |
| **Total** | **58-76** | **43.3%** | **-23.28** |

### Moneyline Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 71-99 | 41.8% | -34.46 |
| 2-3.9% | 7-9 | 43.8% | -2.64 |
| 4-5.9% | 0-6 | 0.0% | -6.00 |
| 6%+ | 1-2 | 33.3% | -1.09 |
| **Total** | **79-116** | **40.5%** | **-44.19** |

### Moneyline Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 48-56 | 46.2% | -12.37 |
| 2-3.9% | 4-7 | 36.4% | -3.36 |
| 4-5.9% | 0-4 | 0.0% | -4.00 |
| 6%+ | 0-1 | 0.0% | -1.00 |
| **Total** | **52-68** | **43.3%** | **-20.73** |

### Over Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 32-32 | 50.0% | -2.91 |
| 2-3.9% | 2-4 | 33.3% | -2.18 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **34-36** | **48.6%** | **-5.09** |

### Over Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 6-6 | 50.0% | -0.55 |
| 2-3.9% | 0-2 | 0.0% | -2.00 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **6-8** | **42.9%** | **-2.55** |

### Under Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 64-48 | 57.1% | +10.18 |
| 2-3.9% | 8-6 | 57.1% | +1.27 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 2-0 | 100.0% | +1.82 |
| **Total** | **74-54** | **57.8%** | **+13.27** |

### Under Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 18-20 | 47.4% | -3.64 |
| 2-3.9% | 6-2 | 75.0% | +3.45 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **24-22** | **52.2%** | **-0.19** |

---

## Individual Model Performance

Performance of each individual predictive model, calculated across all games regardless of edge.

### Individual Model Performance - Spread Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 843-851 | 49.8% | -84.71 |
| Bart Torvik | 616-604 | 50.5% | -44.06 |
| Evan Miya | 873-907 | 49.0% | -113.44 |
| Haslametrics | 838-880 | 48.8% | -118.26 |

### Individual Model Performance - Moneyline Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 1257-453 | 73.5% | +689.61 |
| Bart Torvik | 934-286 | 76.6% | +563.01 |
| Evan Miya | 1330-466 | 74.1% | +742.97 |

### Individual Model Performance - Over Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 824-830 | 49.8% | -80.98 |
| Bart Torvik | 580-586 | 49.7% | -58.78 |
| Evan Miya | 852-882 | 49.1% | -107.53 |
| Haslametrics | 826-852 | 49.2% | -101.17 |

### Individual Model Performance - Under Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 830-824 | 50.2% | -69.53 |
| Bart Torvik | 586-580 | 50.3% | -47.33 |
| Evan Miya | 882-852 | 50.9% | -50.26 |
| Haslametrics | 852-826 | 50.8% | -51.53 |

---

## Methodology

- **Opening Edge**: The difference between our model's predicted probability and the market's implied probability when the game first appeared in our system
- **Consensus**: All individual models agree on the direction of the bet
- **Profit Calculation**: Assumes flat betting at -110 standard juice (+0.909 units per win, -1 unit per loss)
- **Edge Tiers**: Games are bucketed by their opening edge percentage
- **Individual Model Performance**: Each model's performance across all games, regardless of edge

## Notes

- This report updates automatically every 15 minutes
- Only completed games with valid opening edges are included
- Games without opening edge data (e.g., missing from initial scrape) are excluded from this report

