# CBB Model Record

**Last Updated:** January 01, 2026 at 04:04 PM ET

**Season Record Period:** 2025-11-03 to 2026-01-01

**Total Games Graded:** 343 games (686 team-rows)

---

## Record by Opening Edge Tier

The opening edge is calculated when a game first appears in our system, comparing our model's predicted probability against the market's implied probability at that moment.

### Spread Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 50-38 | 56.8% | +7.45 |
| 2-3.9% | 15-25 | 37.5% | -11.36 |
| 4-5.9% | 16-11 | 59.3% | +3.54 |
| 6%+ | 8-11 | 42.1% | -3.73 |
| **Total** | **89-85** | **51.1%** | **-4.10** |

### Spread Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 15-16 | 48.4% | -2.37 |
| 2-3.9% | 10-12 | 45.5% | -2.91 |
| 4-5.9% | 7-7 | 50.0% | -0.64 |
| 6%+ | 2-7 | 22.2% | -5.18 |
| **Total** | **34-42** | **44.7%** | **-11.10** |

### Moneyline Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 64-87 | 42.4% | -28.82 |
| 2-3.9% | 5-9 | 35.7% | -4.46 |
| 4-5.9% | 0-5 | 0.0% | -5.00 |
| 6%+ | 1-1 | 50.0% | -0.09 |
| **Total** | **70-102** | **40.7%** | **-38.37** |

### Moneyline Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 45-49 | 47.9% | -8.09 |
| 2-3.9% | 4-7 | 36.4% | -3.36 |
| 4-5.9% | 0-4 | 0.0% | -4.00 |
| 6%+ | 0-1 | 0.0% | -1.00 |
| **Total** | **49-61** | **44.5%** | **-16.45** |

### Over Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 4-4 | 50.0% | -0.36 |
| 2-3.9% | 0-0 | N/A | +0.00 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **4-4** | **50.0%** | **-0.36** |

### Over Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 0-0 | N/A | +0.00 |
| 2-3.9% | 0-0 | N/A | +0.00 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **0-0** | **N/A** | **+0.00** |

### Under Bets - All

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 24-18 | 57.1% | +3.82 |
| 2-3.9% | 6-4 | 60.0% | +1.45 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 2-0 | 100.0% | +1.82 |
| **Total** | **32-22** | **59.3%** | **+7.09** |

### Under Bets - Consensus Only

| Edge Tier | Record | Win Rate | Profit (Units) |
|-----------|--------|----------|----------------|
| 0-1.9% | 6-12 | 33.3% | -6.55 |
| 2-3.9% | 6-0 | 100.0% | +5.45 |
| 4-5.9% | 0-0 | N/A | +0.00 |
| 6%+ | 0-0 | N/A | +0.00 |
| **Total** | **12-12** | **50.0%** | **-1.10** |

---

## Individual Model Performance

Performance of each individual predictive model, calculated across all games regardless of edge.

### Individual Model Performance - Spread Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 277-321 | 46.3% | -69.21 |
| Bart Torvik | 302-326 | 48.1% | -51.48 |
| Evan Miya | 313-371 | 45.8% | -86.48 |
| Haslametrics | 304-332 | 47.8% | -55.66 |

### Individual Model Performance - Moneyline Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 479-121 | 79.8% | +314.41 |
| Bart Torvik | 512-116 | 81.5% | +349.41 |
| Evan Miya | 550-136 | 80.2% | +363.95 |

### Individual Model Performance - Over Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 268-286 | 48.4% | -42.39 |
| Bart Torvik | 264-312 | 45.8% | -72.02 |
| Evan Miya | 296-338 | 46.7% | -68.94 |
| Haslametrics | 274-318 | 46.3% | -68.93 |

### Individual Model Performance - Under Bets

| Model | Record | Win Rate | Profit (Units) |
|-------|--------|----------|----------------|
| KenPom | 286-268 | 51.6% | -8.03 |
| Bart Torvik | 312-264 | 54.2% | +19.61 |
| Evan Miya | 338-296 | 53.3% | +11.24 |
| Haslametrics | 318-274 | 53.7% | +15.06 |

---

## Methodology

- **Opening Edge**: The difference between our model's predicted probability and the market's implied probability when the game first appeared in our system
- **Consensus**: All individual models agree on the direction of the bet
- **Profit Calculation**: Assumes flat betting at -110 standard juice (+0.909 units per win, -1 unit per loss)
- **Edge Tiers**: Games are bucketed by their opening edge percentage
- **Individual Model Performance**: Each model's performance across all games, regardless of edge

## Notes

- This report updates automatically every 15 minutes
- Only completed games with valid opening edges are included
- Games without opening edge data (e.g., missing from initial scrape) are excluded from this report

