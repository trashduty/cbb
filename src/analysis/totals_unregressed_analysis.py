...
"""
Totals Analysis - UNREGRESSED (using lookup tables)

Edge = lookup_prob - 52.38% (no regression toward 50%)
"""
...
def main():
    ...
    for bet_type in ['over', 'under']:
        ...
        for threshold in thresholds:
            ...
            for combo in combos:
                ...
                # UNREGRESSED: use lookup_prob directly
    ...
    # Generate report
    lines = []
    lines.append("# Totals Analysis - UNREGRESSED (Lookup Tables)")
    lines.append("")
    lines.append("## Methodology")
    lines.append("")
    lines.append("- **Model combination**: Mean (average) of selected model totals")
    lines.append("- **Probability**: Looked up from `totals_lookup_combined.csv`")
    lines.append("- **NO REGRESSION** - uses lookup probability directly")
    lines.append("- **Edge**: lookup_prob - 52.38%")
    ...
    for threshold in thresholds:
        ...
        lines.append("")
        lines.append("| Bet Type | Best Combo | Games | W-L | Accuracy | ROI |")
        lines.append("|----------|------------|-------|-----|----------|-----|")
        ...
        for bet_type in ['over', 'under']:
            ...
            if best:
                ...
                lines.append(f"| {bet_type.upper()} | {combo_name} | {r[0]} | {r[1]}-{r[2]} | {acc:.1f}% | {roi_str} |")
            else:
                lines.append(f"| {bet_type.upper()} | N/A | - | - | - | - |")
    ...
    # Write report
    output_path = base_path / 'analysis' / 'reports' / 'totals_unregressed_analysis.md'
    ...
    for bet_type in ['over', 'under']:
        ...
        for combo in combos:
            combo_name = '+'.join([MODEL_ABBREV[m] for m in combo])
            r = results[bet_type][2][combo_name]
            if r[0] >= 30 and r[3] > best_roi:
                best_roi = r[3]
                best = (combo_name, r)
        if best:
            combo_name, r = best
            print(f"{bet_type.upper()}: {combo_name} - {r[0]} games, {r[1]}-{r[2]}, {r[3]:+.1f}% ROI")
