...
"""
Spreads Analysis - REGRESSED (using lookup tables)
...
def main():
    ...
    for threshold in thresholds:
        ...
        for combo in combos:
            ...
            # REGRESSED: regress forecast toward market before lookup
            df['regressed_spread'] = 0.6 * df['opening_spread'] + 0.4 * df['combo_spread']

            # Lookup probability using regressed spread
            def get_prob(row):
                return lookup_cover_prob(
                    row['total_category'],
                    row['opening_spread'],
                    row['regressed_spread']
                )
    ...
    # Generate report
    lines = []
    lines.append("# Spreads Analysis - REGRESSED (Lookup Tables)")
    lines.append("")
    ...
    lines.append("")
    lines.append("### Step 3: Calculate Edge (REGRESSED)")
    lines.append("- **regressed_spread = 0.6 × market_spread + 0.4 × combo_spread**")
    lines.append("- Look up `cover_prob` using `regressed_spread` (not raw combo spread)")
    lines.append("- **Edge = cover_prob - 52.38%**")
    lines.append("- 52.38% is break-even at -110 odds")
    lines.append("- Regression pulls the model forecast toward the market line before lookup")
    lines.append("")
    ...
    # Write report
    output_path = base_path / 'analysis' / 'reports' / 'spreads_regressed_analysis_points.md'
