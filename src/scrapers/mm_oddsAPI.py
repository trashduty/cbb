...
MM_OUTPUT_PATH = os.path.join(project_root, 'MM_Output_Final.csv')
...
def preserve_opening_odds(new_df, csv_path):
    """
    Preserve opening odds and edge values from the previous MM_Output_Final.csv.
    Keyed on (Game, Team). Mirrors the logic in oddsAPI.preserve_opening_odds().
    """
    if not os.path.exists(csv_path):
        logger.info("[cyan]No existing MM_Output_Final.csv; all values are opening values[/cyan]")
        return new_df
...
def process_mm_dataframe(df):
    ...
    # Predicted Outcome: 50% model / 50% market, rounded to 0.5
    df['Predicted Outcome'] = (
        (df['model_spread'] * 0.5 + df['market_spread'] * 0.5) * 2
    ).round() / 2
...
def main():
    logger.info("=== Starting MM OddsAPI ETL ===")
    ...
    logger.info(f"[green]✓[/green] Saved MM_Output_Final.csv ({len(final_df)} rows) → {MM_OUTPUT_PATH}")
    logger.info("=== MM OddsAPI ETL completed ===")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"[red]✗[/red] MM OddsAPI script failed: {e}")
        import traceback
